...
const Talabalar = () => {
  ...
  const [totalPage] = useState(Math.ceil(studentList?.length / rowsLimit));
  const [currentPage, setCurrentPage] = useState(0);
  const nextPage = () => {
    const startIndex = rowsLimit * (currentPage + 1);
    const endIndex = startIndex + rowsLimit;
    const newArray = students.slice(startIndex, endIndex);
    ...
    <div>
      ...
      <div className="bg-white flex  items-center pt-10 pb-14">
        <div className="w-full max-w-7xl sm:px-1 md:px-2">
          <div className="w-full md:overflow-auto max-w-7xl 2xl:max-w-none mt-2">
            <table className="table-auto overflow-scroll md:overflow-auto w-full text-left font-inter border overflow-x-scroll">
          ...
          <div className="w-full  flex justify-center sm:justify-between flex-col sm:flex-row gap-5 mt-1.5 px-1 items-center">
            <div className="text-lg">
              Ko'rsatilyapti{" "}
              {currentPage === 0 ? 1 : currentPage * rowsLimit + 1} dan{" "}
              {currentPage === totalPage - 1
                ? studentList?.length
                : (currentPage + 1) * rowsLimit}{" "}
              gacha. Jami: {studentList?.length} talaba
            </div>
            <div className="flex">
              <ul
